//%%writefile mergeSmallBatch_shared.cu
#include <stdio.h> 
#include <stdlib.h>
#include <string.h>
...
__device__ void mergeSmallBatch_k(int *A, int *B, int *M, int size_A, int size_B, int size_M)
{
    int i = threadIdx.x;
    if (i< size_M)
    {
      ...
      while (1)
      {
          ...
          if (Q[1] >= 0 && Q[0] <= size_B && (Q[1]== size_A || Q[0]==0 || A[Q[1]]>B[Q[0]-1]))//verif
          {
              if (Q[0]==size_B || Q[1]==0 || A[Q[1]-1]<=B[Q[0]])//verif
              {
                  ...
                  {
                      M[blockIdx.x * blockDim.x + i]=A[Q[1]];
                  }
                  else
                  {
                      M[blockIdx.x * blockDim.x + i]=B[Q[0]]; 
                  }
                  break;
              }
              else
...
__global__ void sortManager(int *A, int *B, int *M, int size_A, int size_B, int size_M)
{
      ...
      __shared__ int shared_AB[1024];  //Taille maximale de thread
      ...
      mergeSmallBatch_k(s_A, s_B, M, size_A, size_B, size_M); 
...
int main(int argc, char *argv[]) {
  ...
  printf("taille alea A:%d, B:%d N:%d \n",h_taille_A,h_taille_B,N);
  ...
  */
  ...
  /*Merge tableau*/
  sortManager<<<numBlock,threadsPerBlock>>>(d_A, d_B, d_M, h_taille_A, h_taille_B, h_taille_M);
