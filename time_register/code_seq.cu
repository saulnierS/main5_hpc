...
int main(int argc, char *argv[]) {
  ...
  srand(42);
  for (int ind_test=10; ind_test<1024; ind_test=ind_test+10)

  {
  /*Déclaration des variables CPU*/
  /*Taille des tableaux*/
  int h_taille_A=ind_test;
  ...
  fprintf(stderr,"Code_sequentiel Taille_A: %d, Taille_B: %d, Taille_M: %d, Temps: %.5f, verif:%d\n", h_taille_A, h_taille_B, h_taille_M,ms,verif_trie(h_M,h_taille_M));
